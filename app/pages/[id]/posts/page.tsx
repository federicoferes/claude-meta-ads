...
export default function PostsPage() {
  ...
  const pageName = searchParams.get("name") ?? id;
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    fetch(`/api/pages/${id}/posts?datePreset=${datePreset}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const avgEngagement = posts.length > 0
    ...
    : 0;
  const boostedCount = posts.filter((p) => p.impressions_paid > 0).length;
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>/</span>
        <Link href="/pages" className="hover:text-white transition-colors">Páginas</Link>
        <span>/</span>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Engagement de Posts Orgánicos</p>
        </div>
        <select
          value={datePreset}
      ...
      {boostedCount > 0 && (
        <p className="text-xs text-gray-500 mb-4">
          {boostedCount} de {posts.length} posts tienen inversión pagada (boosteados).
        </p>
      )}
